...
import { isNativeApp, nativePlatform, requestNativeHealthAccess, readNativeHealthToday } from "./nativeHealth.js";
...
const GOOGLE_CLIENT_ID = "619052415275-6cs36e9mml9h8tdb9cl0g2tnhf06ic9f.apps.googleusercontent.com";
...
const Card=({children,style,onClick,...p})=><div onClick={onClick} className={onClick?"jz-press":undefined} style={{background:`linear-gradient(165deg, ${C.sf} 0%, ${C.bg} 130%)`,border:`1px solid ${C.bd}`,borderRadius:18,padding:20,boxShadow:"0 6px 20px rgba(0,0,0,.22)",cursor:onClick?"pointer":undefined,...style}} {...p}>{children}</div>;
...
const Btn=({children,variant="primary",style,disabled,...p})=>{const v={primary:{background:C.gr,color:"#fff",boxShadow:`0 6px 18px ${C.ac}45`},secondary:{background:C.s2,color:C.tx,border:`1px solid ${C.bd}`},danger:{background:C.dg+"22",color:C.dg},ghost:{background:"transparent",color:C.mt}};return<button className="jz-press" style={{padding:"12px 24px",borderRadius:12,border:"none",fontWeight:700,fontSize:14,cursor:disabled?"not-allowed":"pointer",opacity:disabled?.5:1,fontFamily:"inherit",display:"inline-flex",alignItems:"center",gap:8,justifyContent:"center",letterSpacing:".1px",...v[variant],...style}} disabled={disabled}{...p}>{children}</button>;};
...
const SC=({label,value,icon,color})=><Card style={{padding:16,position:"relative",overflow:"hidden"}}><div style={{position:"absolute",top:-20,right:-20,width:70,height:70,borderRadius:"50%",background:color,opacity:.16,filter:"blur(18px)"}}/><div style={{width:38,height:38,borderRadius:11,background:color+"20",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:10,fontSize:18,boxShadow:`0 4px 14px ${color}40`,position:"relative"}}>{icon}</div><div style={{fontSize:23,fontWeight:800,color:C.tx,letterSpacing:"-.3px",position:"relative"}}>{value}</div><div style={{fontSize:11.5,color:C.mt,marginTop:2,fontWeight:500,position:"relative"}}>{label}</div></Card>;
...
function AuthScreen(){const{login,register,googleLogin}=useAuth();const[mode,setMode]=useState("login");const[form,setForm]=useState({name:"",email:"",password:"",role:"CLIENT",phone:""});const[error,setError]=useState("");const[success,setSuccess]=useState("");const[busy,setBusy]=useState(false);const[resetToken,setResetToken]=useState("");const[newPassword,setNewPassword]=useState("");const[resetMethod,setResetMethod]=useState("email");
  ...
  useEffect(()=>{
    ...
    if(GOOGLE_CLIENT_ID.startsWith("YOUR_GOOGLE_CLIENT_ID")){log("Set GOOGLE_CLIENT_ID in App.jsx to enable Google Sign-In");return;}
    let cancelled=false;
    const tryRender=()=>{
    ...
    if(tryRender())return;
    // Script wasn't ready yet — poll every 200ms for up to ~6s, then give up quietly.
    let attempts=0;
...
// ─── DASHBOARD ────────────────────────────────────────────────────────────────
function DailyBriefing({onNav}){
  const[data,setData]=useState(null);const[loading,setLoading]=useState(true);const[dismissed,setDismissed]=useState(false);
  ...
  return<div style={{borderRadius:20,padding:2,background:C.gr,marginBottom:16}}>
...
// ─── INSIGHT SETTINGS ─────────────────────────────────────────────────────
// Lets a coach tune the Daily Briefing / risk-flag thresholds to match how
// THEY actually coach — a twice-weekly bootcamp coach and a monthly-checkin
// online coach have very different definitions of "gone quiet." Backed by
// GET/PUT /api/insights/settings (see services/insightsService.js).
...
function InsightSettingsPage(){
  const[settings,setSettings]=useState(null);const[bounds,setBounds]=useState({});const[defaults,setDefaults]=useState({});
  const[loading,setLoading]=useState(true);const[saving,setSaving]=useState(false);const[saved,setSaved]=useState(false);
  useEffect(()=>{api.get("/insights/settings").then(r=>{setSettings(r.settings);setBounds(r.bounds||{});setDefaults(r.defaults||{});}).catch(()=>{}).finally(()=>setLoading(false));},[]);
  const update=(key,val)=>{setSettings(s=>({...s,[key]:val}));setSaved(false);};
  const resetAll=()=>{setSettings({...defaults});setSaved(false);};
  const save=async()=>{setSaving(true);try{const r=await api.put("/insights/settings",settings);setSettings(r.settings);setSaved(true);setTimeout(()=>setSaved(false),2500);}catch(e){alert("Failed to save: "+e.message);}setSaving(false);};
  ...
  return<div>
    ...
    {INSIGHT_FIELDS.map(f=>{const b=bounds[f.key]||{min:0,max:100};const val=settings[f.key]??defaults[f.key];return<Card key={f.key} style={{marginBottom:12}}>
      ...
      <input type="range" min={b.min} max={b.max} value={val} onChange={e=>update(f.key,+e.target.value)}
...
function DashboardPage({onNav}){const{user}=useAuth();const[stats,setStats]=useState({});const[up,setUp]=useState([]);const[clientCount,setClientCount]=useState(0);const[leadCount,setLeadCount]=useState(0);const[loading,setLoading]=useState(true);useEffect(()=>{Promise.all([api.get("/reports/coach/dashboard").catch(()=>({})),api.get("/bookings").catch(()=>({})),api.get("/clients").catch(()=>({})),api.get("/leads").catch(()=>({}))]).then(([s,b,c,l])=>{setStats(s?.data||s||{});const cl=unwrap(c,"clients");setClientCount(cl.length);const ld=unwrap(l,"leads");setLeadCount(ld.length);const allBk=unwrap(b,"bookings","sessions");const now=new Date();setUp(allBk.filter(x=>{try{const st=(x.status||"").toUpperCase();return new Date(x.date||x.startTime||x.scheduledAt)>=now&&st!=="CANCELLED"&&st!=="ABSENT";}catch{return false;}}).sort((a,b)=>new Date(a.date||a.startTime||a.scheduledAt)-new Date(b.date||b.startTime||b.scheduledAt)).slice(0,5));}).finally(()=>setLoading(false));},[]);if(loading)return<Spin/>;const g=new Date().getHours()<12?"Good morning":new Date().getHours()<17?"Good afternoon":"Good evening";const go=id=>{if(onNav)onNav(id);};return<div style={{position:"relative"}}><div style={{position:"absolute",top:-24,left:-16,right:-16,height:200,background:`radial-gradient(circle at 20% 15%, ${C.ac}45, transparent 55%), radial-gradient(circle at 85% 5%, ${C.a2}35, transparent 50%)`,filter:"blur(14px)",pointerEvents:"none",zIndex:0}}/><div style={{position:"relative",zIndex:1}}><div style={{marginBottom:20}}><div style={{fontSize:14,color:C.mt,fontWeight:500}}>{g},</div><h2 style={{background:`linear-gradient(90deg, ${C.tx} 40%, ${C.ac} 130%)`,WebkitBackgroundClip:"text",backgroundClip:"text",color:"transparent",fontSize:24,margin:"2px 0 0",fontWeight:800,letterSpacing:"-.3px"}}>{user?.name||"Coach"} 👋</h2></div><DailyBriefing onNav={go}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><div onClick={()=>go("clients")} style={{cursor:"pointer"}}><SC label="Active Clients" value={clientCount} icon="👥" color={C.ac}/></div><div onClick={()=>go("reports")} style={{cursor:"pointer"}}><SC label="Monthly Revenue" value={`₹${(stats.monthlyRevenue??stats.totalRevenue??0).toLocaleString()}`} icon="📈" color={C.ok}/></div><div onClick={()=>go("bookings")} style={{cursor:"pointer"}}><SC label="Upcoming" value={up.length} icon="📅" color={C.a2}/></div><div onClick={()=>go("leads")} style={{cursor:"pointer"}}><SC label="Leads" value={leadCount} icon="🎯" color={C.wn}/></div></div><Card style={{marginTop:16,cursor:"pointer"}} onClick={()=>go("bookings")}><div style={{fontSize:15,fontWeight:600,color:C.tx,marginBottom:12}}>Upcoming Sessions</div>{up.length===0?<div style={{color:C.mt,fontSize:13}}>No upcoming sessions</div>:up.map(s=><div key={s.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:`1px solid ${C.bd}`}}><div style={{width:40,height:40,borderRadius:10,background:C.ac+"18",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>📅</div><div style={{flex:1}}><div style={{fontSize:14,fontWeight:600,color:C.tx}}>{cName(s.client)||s.type||"Session"}</div><div style={{fontSize:12,color:C.mt}}>{new Date(s.date||s.startTime||s.scheduledAt).toLocaleDateString()} · {s.duration||60}min</div></div><Badge color={(s.status||"").toLowerCase()==="confirmed"?C.ok:C.wn}>{(s.status||"pending").toLowerCase()}</Badge></div>)}</Card></div></div>;}

// ─── CLIENTS ──────────────────────────────────────────────────────────────────
function ClientsPage(){
  const[clients,setClients]=useState([]);const[loading,setLoading]=useState(true);const[risks,setRisks]=useState({});const[expandedRisk,setExpandedRisk]=useState(null);
  ...
  return<div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      {filtered.map(c=>{const isOnline=c.lastLogin&&(Date.now()-new Date(c.lastLogin).getTime())<15*60*1000;const lastSeen=c.lastLogin?new Date(c.lastLogin):c.lastActive?new Date(c.lastActive):null;const lastSeenText=lastSeen?((Date.now()-lastSeen.getTime())<60*60*1000?`${Math.round((Date.now()-lastSeen.getTime())/60000)}m ago`:lastSeen.toLocaleDateString()):"Never";const risk=risks[c.id];const isExpanded=expandedRisk===c.id;return<Card key={c.id} onClick={()=>setSel(c)} style={{padding:14,cursor:"pointer",...(risk?.flagged?{borderColor:C.wn+"60"}:{})}}>
      ...
      {risk?.flagged&&isExpanded&&<div style={{marginTop:10,paddingTop:10,borderTop:`1px dashed ${C.bd}`}}>
...
// device — a raw phone camera photo can be 3-8MB; nothing about a food-log
// thumbnail needs that. Keeps payload small and Postgres storage sane.
...
function NutritionTracker({cid}){
  ...
  const onPhotoSelected=async(e)=>{
    ...
    try{
      ...
      setPhotoStep("review"); // fall through to manual review even if AI analysis failed
    }
  };
  ...
  return<div>
    ...
    tm.map(m=><Card key={m.id} style={{padding:12,marginBottom:6,display:"flex",gap:10,alignItems:"center"}}>
    ...
    <Modal open={showPhoto} onClose={()=>setShowPhoto(false)} title="Log Food with Photo" wide>
      {photoStep==="capture"&&<div style={{display:"flex",flexDirection:"column",gap:14,alignItems:"center",padding:"10px 0"}}>
      ...
      {photoStep==="analyzing"&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:14,padding:"30px 0"}}>
      ...
      {photoStep==="review"&&<div style={{display:"flex",flexDirection:"column",gap:12}}>
...
function BookingsPage(){
  ...
  return<div>
    ...
    {/* ── WEEK VIEW ── */}
    {viewMode==="week"&&<div className="jz-scrollx" style={{display:"flex",gap:6,marginBottom:16,paddingBottom:2}}>
      {(()=>{const b=new Date(selDate);const s=new Date(b);s.setDate(b.getDate()-b.getDay()-7);return Array.from({length:21},(_,i)=>{const d=new Date(s);d.setDate(s.getDate()+i);return d;});})().map((d,i)=>{
        ...
        return<button key={i} onClick={()=>setSelDate(iso)} style={{flexShrink:0,width:52,padding:"10px 2px",borderRadius:12,border:"none",cursor:"pointer",background:isSel?C.gr:isH?C.dg+"20":C.s2,display:"flex",flexDirection:"column",alignItems:"center",gap:4,boxShadow:isSel?`0 4px 14px ${C.ac}45`:"none"}}>
...
User question: ${msg}`;
  ...
  return<div style={{display:"flex",flexDirection:"column",height:"calc(100dvh - 120px)",maxHeight:"calc(100dvh - 120px)",position:"relative"}}>
    ...
    {/* ── Business Quick Prompts (coach only, shown before they've asked anything) ── */}
    {!loading&&user?.role==="COACH"&&msgs.length===1&&<div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:8,flexShrink:0}}>
      {["Who needs attention?","How's this month looking?","Which leads should I prioritize?","Best time to add a client?"].map(q=>
        <button key={q} onClick={()=>send(q)} style={{flexShrink:0,background:C.s2,border:`1px solid ${C.bd}`,color:C.tx,fontSize:12,fontWeight:600,padding:"8px 13px",borderRadius:20,whiteSpace:"nowrap",cursor:"pointer",fontFamily:"inherit"}}>{q}</button>
      )}
    </div>}
...
function BNav({active,onChange}){const tabs=TABS();return<nav style={{position:"fixed",bottom:0,left:0,right:0,zIndex:100,background:C.sf,borderTop:`1px solid ${C.bd}`,display:"flex",paddingBottom:"env(safe-area-inset-bottom,0px)"}}>{tabs.map(t=>{const a=active===t.id||(active==="more"&&false);return<button key={t.id} onClick={()=>onChange(t.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4,padding:"12px 0 10px",border:"none",cursor:"pointer",background:"transparent",minHeight:60}}><div style={{padding:"5px 14px",borderRadius:12,background:a?C.ac+"20":"transparent",fontSize:23,lineHeight:1}}>{t.icon}</div><span style={{fontSize:11.5,fontWeight:a?700:500,color:a?C.ac:C.mt}}>{t.label}</span></button>;})}
  {/* More button — always last, not movable */}
  <button onClick={()=>onChange("more")} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4,padding:"12px 0 10px",border:"none",cursor:"pointer",background:"transparent",minHeight:60}}><div style={{padding:"5px 14px",borderRadius:12,background:active==="more"?C.ac+"20":"transparent",fontSize:23,lineHeight:1}}>⚙️</div><span style={{fontSize:11.5,fontWeight:active==="more"?700:500,color:active==="more"?C.ac:C.mt}}>More</span></button>
  </nav>;}
...
function FitnessDevicesPage(){
  ...
  const toggleConnect=async(id)=>{
    ...
    if(dev.type==="oauth"){
      ...
      // Apple Health / Health Connect — real native read when running inside
      // the compiled app; otherwise explain why it can't work in the browser.
      if(!isNativeApp()){
        alert(`${dev.name}\n\n${dev.note}`);
    ...
    }else if(dev.type==="deprecated"){
      alert(`${dev.name}\n\n${dev.note}`);
    }else if(dev.type==="info"){
      alert(`${dev.name}\n\n${dev.note}`);
    }else{
      // manual-only: no API exists anywhere for this device — go straight to logging
      alert(`${dev.name}\n\n${dev.note}`);
      setManualSource(id);setShowManual(true);
    }
  };
  ...
  return<div>
    <ST right={<Btn onClick={()=>{setManualSource("manual");setShowManual(true);}} style={{padding:"8px 12px",fontSize:12}}>✏️ Manual</Btn>}>
    ...
    {tab==="connect"&&<div style={{display:"flex",flexDirection:"column",gap:8}}>
      {devices.map(d=>{
        ...
        return<Card key={d.id} style={{padding:14,display:"flex",alignItems:"center",gap:14}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:4,alignItems:"flex-end"}}>
          <button onClick={()=>toggleConnect(d.id)} style={{padding:"8px 16px",borderRadius:10,border:"none",cursor:"pointer",fontSize:12,fontWeight:600,whiteSpace:"nowrap",background:connections[d.id]?C.ok+"20":C.ac+"20",color:connections[d.id]?C.ok:C.ac}}>
            {btnLabel}
          </button>
...
function ClientMainApp(){
  ...
  return<div style={{minHeight:"100dvh",background:C.bg,color:C.tx,fontFamily:"'DM Sans','SF Pro Display',-apple-system,system-ui,sans-serif"}}>
    ...
    <nav style={{position:"fixed",bottom:0,left:0,right:0,zIndex:100,background:C.sf,borderTop:`1px solid ${C.bd}`,display:"flex",paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      {clientTabs.map(t=>{const a=tab===t.id;return<button key={t.id} onClick={()=>{setTab(t.id);setRk(k=>k+1);}} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4,padding:"12px 0 10px",border:"none",cursor:"pointer",background:"transparent",minHeight:60}}>
      ...
      {[{id:"photos",icon:"📸",label:"Photos"},{id:"settings",icon:"⚙️",label:"Settings"}].map(t=>{const a=tab===t.id;return<button key={t.id} onClick={()=>{setTab(t.id);setRk(k=>k+1);}} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4,padding:"12px 0 10px",border:"none",cursor:"pointer",background:"transparent",minHeight:60}}>
        <div style={{padding:"5px 14px",borderRadius:12,background:a?C.ac+"20":"transparent",fontSize:23,lineHeight:1}}>{t.icon}</div>
        <span style={{fontSize:11.5,fontWeight:a?700:500,color:a?C.ac:C.mt}}>{t.label}</span>
      </button>;})}
    </nav>
