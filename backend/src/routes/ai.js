...
router.post("/chat", authenticate, aiLimiter, sanitizeBody, async (req, res) => {
  try {
    ...
    // Ground the assistant in server-verified data (not just whatever the
    // client happened to fetch and stuff into `system`) — this is what
    // turns "which clients need attention" from a generic answer into one
    // backed by the same signals the Daily Briefing uses. Coach-only: a
    // client's own chat doesn't get another coach's business data.
    let groundedSystem = system || "";
    if (req.user.role === "COACH") {
      try {
        const coach = await profileRepository.findCoachProfileByUserId(req.user.id);
        if (coach) groundedSystem += await insightsService.getGroundedContext(coach.id, req.user.id);
    ...
    res.json({ text, usage: data.usage, grounded: !!groundedSystem && req.user.role === "COACH" });
  } catch (err) {
